...
//!
//! ```
//! use argy::{FromArgs, FromArgValue};
//!
//! #[derive(FromArgValue)]
//! enum Mode {
//!     SoftCore,
...
//!
//! ```rust
//! # use argy::CommandInfo;
//! # use argy::DynamicSubCommand;
//! # use argy::EarlyExit;
//! # use argy::FromArgs;
...
//!
//!             // argy needs the `CommandInfo` structs we generate to be valid
...
pub type CommandInfoWithArgs = argy_shared::CommandInfoWithArgs<'static>;

/// Information about a subcommand.
pub type SubCommandInfo = argy_shared::SubCommandInfo<'static>;

pub use argy_shared::{ErrorCodeInfo, FlagInfo, FlagInfoKind, Optionality, PositionalInfo};
...
pub trait FromArgs: Sized {
    ...
    /// allows `argy` to generate correct subcommand help strings.
    ///
...
/// Create a `FromArgs` type from the current process's `env::args`.
///
/// This special cases usages where argy is being used in an environment where cargo is
