...
fn test_no_short_alias_for_two() {
    // "two" has no short alias, so "t" should fail (unless "t" is prefix matching? argy doesn't do prefix matching for subcommands by default I think, but let's see)
    // Actually argy requires exact match for subcommands unless strict is disabled?
    // Let's assume strict.
